...
class ProductManager {
  ...
  async syncFromSheets() {
    try {
      ...
      const response = await GoogleSheetsAPI.getAllProducts();
      ...
      console.log(`Successfully synced ${this.localProducts.length} products from sheets`);
  ...
  async bulkUpdateProducts(updates) {
    try {
      console.log('Bulk updating products:', updates.length);

      let result;
      if (this.isOnline) {
        // Convert all updates to sheet format
        const sheetUpdates = updates.map(update => ({
          id: update.id,
          data: this.convertToSheetFormat(update.data)
        }));
        
        result = await GoogleSheetsAPI.bulkUpdateProducts(sheetUpdates);
        ...
        // Handle bulk update in offline mode
        const results = [];
        for (const update of updates) {
          const localResult = await this.updateProduct(update.id, update.data);
          results.push({ id: update.id, result: localResult });
        }
  ...
  // Convert sheet product format to app format
  convertToAppFormat(sheetProduct) {
    return {
      id: sheetProduct.id || Date.now(),
      name: sheetProduct.name || '',
      description: sheetProduct.description || `${sheetProduct.name || 'Product'} - Perfect for your needs.`,
