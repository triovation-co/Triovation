...
export default function CustomizationPage() {
  ...
  const customizationDetails = `Power: ${powerSources.find((p) => p.value === selectedPower)?.label || "—"
    }, LED: ${ledColors.find((c) => c.value === selectedColor)?.label || "—"
    }, Design: ${lampDesigns.find((d) => d.value === selectedDesign)?.label || "—"
    }`;
  ...
  const handleAddToCart = () => {
    ...
    window.dispatchEvent(new CustomEvent('cart-item-added', {
      detail: { name: `${product.name} (Customized)`, image: product.image, price: product.price }
    }));
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {powerSources.map((power) => (
                <button
                  ...
                  className={`relative rounded-lg overflow-hidden border transition-all ${selectedPower === power.value
                      ? "border-gray-900"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {ledColors.map((color) => (
                <button
                  ...
                  className={`relative rounded-lg overflow-hidden border transition-all ${selectedColor === color.value
                      ? "border-gray-900"
                      : "border-gray-200 hover:border-gray-400"
                    }`}
                >
              ...
              >
                {lampDesigns.map((design, idx) => (
                  <button
                    ...
                    className={`relative flex-shrink-0 rounded-lg overflow-hidden border transition-all ${selectedDesign === design.value
                        ? "border-gray-900"
                        : "border-gray-200 hover:border-gray-400"
                      }`}
                    style={{
